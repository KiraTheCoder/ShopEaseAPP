                    ...
                    {cartData?.length > 0 ? cartData?.map((product) => (
                        <div key={product._id} className='flex justify-between border-b-2 p-2'>
                            <div className='flex gap-2'>
                                <img
                                    // src={  product?.images[0]? `data:${product?.images[0]?.contentType};base64,${product?.images[0]?.data}`:null}
                                    src={product?.images?.[0] ? `data:${product.images[0].contentType};base64,${product.images[0].data}` : defaultImage}
