...
function Header() {
    ...
    return (
        <header className="bg-red-100 py-4">
            <div className="container mx-auto flex flex-col md:flex-row items-center justify-between px-4">
                ...
                <nav className={`md:flex md:items-center md:space-x-4 mt-4 md:mt-0 ${isMenuOpen ? 'block' : 'hidden'}`}>
                    ...
                    <NavLink to={`/${activeLink}`} className={({ isActive }) => `text-sm sm:text-base md:text-lg font-medium text-white relative after:content-[''] after:absolute after:left-0 after:bottom-0 after:h-1 after:w-1/2 after:bg-[#db4444] ${isActive ? 'after:block' : 'after:hidden'} mb-2 md:mb-0`} onClick={() => { activeLink === "signup" ? handleToggle('login') : handleToggle("signup") }}>
                        {activeLink}
                    </NavLink>
